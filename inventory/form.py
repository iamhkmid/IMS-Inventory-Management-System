...
class BarangForm(ModelForm):
    ...
    id_barang = forms.CharField(required=True, widget=forms.TextInput(
        attrs={'autofocus': 'autofocus', 'placeholder': 'xxxxxxx (Length=7)', 'minlength': '7', 'maxlength': '7'}))
    satuan = forms.ChoiceField(required=True, choices=SATUAN)
    jenis = forms.TypedChoiceField(choices=JENIS, widget=forms.RadioSelect())
    tgl_pengadaan = forms.DateTimeField(widget=forms.DateTimeInput(
    ...
    def clean(self):
        ...
        for value1, value2 in barang_list:
            if id_barang == value1:
                self.add_error(
                    'id_barang', "ID barang sudah ada. (" + value1 + " - " + value2 + ")")
            if nama == value2:
                self.add_error(
                    'nama', "Nama barang sudah ada. (" + value1 + " - " + value2 + ")")
...
class UpdateBarangForm(ModelForm):
    ...
    nama = forms.CharField(required=True, widget=forms.TextInput(
...
class AddExistingForm(Form):
    id_barang = forms.ModelChoiceField(
        required=True, queryset=Barang.objects.filter(jenis='Persediaan'))
...
class ConditionUpdateForm(ModelForm):
    jumlah_b = forms.IntegerField(widget=forms.NumberInput(
        attrs={'autofocus': 'autofocus', 'min': 0, }), required=True)
    jumlah_rr = forms.IntegerField(
        widget=forms.NumberInput(attrs={'min': 0, }), required=True)
    jumlah_rb = forms.IntegerField(
        widget=forms.NumberInput(attrs={'min': 0, }), required=True)
    jumlah_hl = forms.IntegerField(
        widget=forms.NumberInput(attrs={'min': 0, }), required=True)
        ...
        if jumlah_b is not None and jumlah_rr is not None:
            ...
            if total1 != total2:
                ...
                raise forms.ValidationError(
                    'Tidak Sesuai dengan total seluruh.')
